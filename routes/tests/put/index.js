...
router.put('/json/:id', (req, res, next) => {
...
router.put('/formdata/:id', upload.any(), (req, res, next) => {
    if (!isNaN(req.params.id)) {
        if (req.headers['content-type'] && req.headers['content-type'].startsWith("multipart/form-data")) {
            res.status(200).send(`Item with id ${req.params.id} edited with new data = ${JSON.stringify(req.body)} | files = ${JSON.stringify(req.files)}`)
        } else {
...
router.put('/urlencoded/:id', (req, res, next) => {
    if (!isNaN(req.params.id)) {
        if (req.headers['content-type'] === 'application/x-www-form-urlencoded') {
            res.status(200).send(`Item with id ${req.params.id} edited with new data = ${JSON.stringify(req.body)}`)
        } else {
